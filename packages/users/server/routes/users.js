...
var captureRedirect = function(req, res, next) {
  req.session.redirect_to = '/#!/' + (req.query.ref ? req.query.ref : '');
  next();
};
...
module.exports = function(MeanUser, app, auth, database, passport) {
  ...
  app.route('/users')
    .get(auth.requiresLogin, hasAuthorization, users.all);
  app.route('/logout')
    .get(users.signout);
  app.route('/users/me')
    .get(users.me);
  ...
  app.route('/login')
    .post(passport.authenticate('local', {
      ...
      res.send({
        user: req.user,
        redirect: '/#!/' + req.query.ref //(req.user.roles.indexOf('admin') !== -1) ? req.query.ref : false
      });
    });
  ...
  app.route('/auth/facebook')
    .get(captureRedirect, passport.authenticate('facebook', {
  ...
  app.route('/auth/instagram/callback')
    .get(users.subscriptionCallback);
