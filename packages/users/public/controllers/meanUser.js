...
angular.module('mean.users')
  ...
  .controller('LoginCtrl', ['$scope', '$rootScope', '$http', '$location', '$window', 'Global',
    function($scope, $rootScope, $http, $location, $window, Global) {
      // This object will be filled by the form
      $scope.user = {};
      $scope.ref = ($location.search().ref ? '?ref=' + $location.search().ref : '');
      $scope.global = Global;
      ...
      $scope.login = function() {
        $http.post('/login' + ($location.search().ref ? '?ref=' + $location.search().ref : ''), {
        ...
        .success(function(response) {
          ...
          $scope.global.isAdmin = $rootScope.user.roles.indexOf('admin') !== -1;
      ...
      $scope.logout = function() {
        $http.get('/logout')
          ...
          .error(function() {
            $scope.loginerror = 'Logout failed.';
          });
      };
    }
